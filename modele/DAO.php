...
class DAO
{
    ...
    public function creerUnUtilisateur($unUtilisateur) {
        ...
        // liaison de la requête et de ses paramètres
        $req1->bindValue("pseudo", mb_convert_encoding($unUtilisateur->getPseudo(), 'ISO-8859-1', 'UTF-8'), PDO::PARAM_STR);
        $req1->bindValue("mdpSha1", mb_convert_encoding(sha1($unUtilisateur->getMdpsha1()), 'ISO-8859-1', 'UTF-8'), PDO::PARAM_STR);
        $req1->bindValue("adrMail", mb_convert_encoding($unUtilisateur->getAdrmail(), 'ISO-8859-1', 'UTF-8'), PDO::PARAM_STR);
        $req1->bindValue("numTel", mb_convert_encoding($unUtilisateur->getNumTel(), 'ISO-8859-1', 'UTF-8'), PDO::PARAM_STR);
        $req1->bindValue("niveau", mb_convert_encoding($unUtilisateur->getNiveau(), 'ISO-8859-1', 'UTF-8'), PDO::PARAM_INT);
        $req1->bindValue("dateCreation", mb_convert_encoding($unUtilisateur->getDateCreation(), 'ISO-8859-1', 'UTF-8'), PDO::PARAM_STR);
        // exécution de la requête
        $ok = $req1->execute();
    ...
    public function  existeAdrMailUtilisateur($mail)
    {
        // préparation de la requête de recherche
        $txt_req = "Select count(*) from tracegps_utilisateurs where adrMail = :mail";
        $req = $this->cnx->prepare($txt_req);
        // liaison de la requête et de ses paramètres
    ...
    public function  creerUnPointDeTrace($unPoint)
    {
        ...
        $req = $this->cnx->prepare($txt_req);
        // liaison de la requête et de ses paramètres
        $req->bindValue("idTrace", $unPoint->getIdTrace(), PDO::PARAM_STR);
        $req->bindValue("id", $unPoint->getId(), PDO::PARAM_STR);
        $req->bindValue("latitude", $unPoint->getLatitude(), PDO::PARAM_STR);
        $req->bindValue("longitude", $unPoint->getLongitude(), PDO::PARAM_STR);
        $req->bindValue("altitude", $unPoint->getAltitude(), PDO::PARAM_STR);
        $req->bindValue("dateHeure", $unPoint->getDateHeure(), PDO::PARAM_STR);
        $req->bindValue("rythmeCardio", $unPoint->getRythmeCardio(), PDO::PARAM_STR);
        // exécution de la requête
        $req->execute();
        $ok = $req->fetch();
        // libère les ressources du jeu de données
