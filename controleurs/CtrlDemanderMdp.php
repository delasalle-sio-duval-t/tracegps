...
if ( ! isset ($_POST ["txtPseudo"]) == true || ! isset ($_POST ["txtAdrMail"]) == true ) {
    ...
    if ($pseudo == '' || $adrMail == '') {
		...
		if ( ! $dao->existePseudoUtilisateur($pseudo) )  {
            ...
            $adrMailUtilisateur = $dao->getUnUtilisateur($pseudo)->getAdrMail();
			if ($adrMail != $adrMailUtilisateur) {
                $message = "Erreur : l'adresse mail ne correspond pas.";
                $typeMessage = 'avertissement';
                $themeFooter = $themeProbleme;
                unset($dao);		// fermeture de la connexion à MySQL
                include_once ('vues/VueDemanderMdp.php');
